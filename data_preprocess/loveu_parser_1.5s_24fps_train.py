...
data_target = "/home/notebook/data/personal/loveu/data/loveu_wide_train/"
anno_file = open("loveu_wide_train_annotation.txt", 'w')
anno_cmds = open("loveu_wide_train_cmds.txt", 'w')
cmds = []
    ...
    for filename in dict_raw.keys():
        ...
        for i in range(len(mr345[filename]['substages_timestamps_all'])):
            ...
            if timestamp - timestamp_last > gap_neg:
                ...
                for i in range(100): # add 1000ms at a time
                    ...
                    if this_time_end > timestamp:
                        break
                    cmd = "ffmpeg -ss " + str(max(0, (this_time_beg))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=24, scale=-2:256\"" + \
            ...
            if timestamp - timestamp_last < gap_pos:
                continue
            cmd = "ffmpeg -ss " + str(max(0, (timestamp - 750))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=24, scale=-2:256\"" + \
                                " -to " + str(1500) + "ms " + data_target + filename + "_s" + str(timestamp) + "_t1500_pos_" + str(label) + ".mp4"
            anno_file.write(data_target + filename + "_s" + str(timestamp) + "_t1500_pos_" + str(label) + ".mp4 " + str(label) + "\n")
            anno_cmds.write(cmd+"\n")
            cmds.append(cmd)
            cn[label] += 1
