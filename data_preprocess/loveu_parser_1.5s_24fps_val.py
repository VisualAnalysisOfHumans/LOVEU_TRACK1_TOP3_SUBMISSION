...
data_target = "/home/notebook/data/personal/loveu/data/loveu_wide_val_fix/"
anno_file = open("loveu_wide_val_annotation_fix.txt", 'w')
anno_cmds = open("loveu_wide_val_cmds_fix.txt", 'w')
cmds = []
    ...
    for filename in dict_raw.keys():
        ...
        for i in range(len(mr345[filename]['substages_timestamps_all'])):
            ...
            if timestamp - timestamp_last > gap_neg:
                ...
                for i in range(100): # add 1000ms at a time
                    ...
                    if this_time_end > timestamp:
                        break
                    cmd = "ffmpeg -ss " + str(max(timestamp_bias, (this_time_beg + timestamp_bias))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=24, scale=-2:256\"" + \
            ...
            if timestamp - timestamp_last < gap_pos:
                continue
            cmd = "ffmpeg -ss " + str(max(timestamp_bias, (timestamp - 750 + timestamp_bias))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=24, scale=-2:256\"" + \
