...
cn = [0, 0, 0, 0]
data_root = "/home/notebook/data/personal/loveu/data/trainset/"
data_target = "/home/notebook/data/personal/loveu/data/loveu_wide_train_2s_30fps/"
anno_file = open("loveu_wide_train_2s_30fps_annotation.txt", 'w')
anno_cmds = open("loveu_wide_train_2s_30fps_cmds.txt", 'w')
cmds = []
...
def generate_frameidx_from_raw(min_change_duration=0.3, split='valnew'):
    ...
    for filename in dict_raw.keys():
        ...
        file_path = data_root + dict_raw[filename]['path_video']
        mr345[filename] = {}
        ...
        for i in range(len(mr345[filename]['substages_timestamps_all'])):
            ...
            if timestamp - timestamp_last > gap_neg:
                ...
                for i in range(100): # add 1000ms at a time
                    ...
                    if this_time_end > timestamp:
                        break
                    cmd = "ffmpeg -ss " + str(max(0, (this_time_beg))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=30, scale=-2:256\"" + \
            ...
            if timestamp - timestamp_last < gap_pos:
                continue
            cmd = "ffmpeg -ss " + str(max(0, (timestamp - 1000))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=30, scale=-2:256\"" + \
                                " -to " + str(2000) + "ms " + data_target + filename + "_s" + str(timestamp) + "_t2000_pos_" + str(label) + ".mp4"
            anno_file.write(data_target + filename + "_s" + str(timestamp) + "_t2000_pos_" + str(label) + ".mp4 " + str(label) + "\n")
            anno_cmds.write(cmd+"\n")
            cmds.append(cmd)
            cn[label] += 1
