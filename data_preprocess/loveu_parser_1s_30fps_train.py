...
data_target = "/home/notebook/data/personal/loveu/data/loveu_wide_train_1s_30fps/"
anno_file = open("loveu_wide_train_1s_30fps_annotation.txt", 'w')
anno_cmds = open("loveu_wide_train_1s_30fps_cmds.txt", 'w')
cmds = []
    ...
    for filename in dict_raw.keys():
        ...
        for i in range(len(mr345[filename]['substages_timestamps_all'])):
            ...
            if timestamp - timestamp_last < gap_pos:
                continue
            cmd = "ffmpeg -ss " + str(max(0, (timestamp - 500))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=30, scale=-2:256\"" + \
