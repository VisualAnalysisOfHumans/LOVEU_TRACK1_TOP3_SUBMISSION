...
data_target = "/home/notebook/data/personal/loveu/data/loveu_wide_val_2s_30fps/"
anno_file = open("loveu_wide_val_2s_30fps_annotation.txt", 'w')
anno_cmds = open("loveu_wide_val_2s_30fps_cmds.txt", 'w')
cmds = []
...
def generate_frameidx_from_raw(min_change_duration=0.3, split='valnew'):
    ...
    for filename in dict_raw.keys():
        ...
        timestamp_bias = 1000 # -1s to 11s in val
        gap_neg = 2000 # 0.6s
        stride_neg = 100
        gap_pos = 300 # 0.1s
        for i in range(len(mr345[filename]['substages_timestamps_all'])):
            ...
            if timestamp - timestamp_last < gap_pos:
                continue
            cmd = "ffmpeg -ss " + str(max(timestamp_bias, (timestamp - 1000 + timestamp_bias))) + "ms -i \"" + file_path + "\" -crf 10 -an -vf \"fps=30, scale=-2:256\"" + \
