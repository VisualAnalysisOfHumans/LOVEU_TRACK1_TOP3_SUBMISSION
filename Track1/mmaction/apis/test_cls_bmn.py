...
def single_gpu_test_bmn(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            _, result_bmn = model(return_loss=False, **data)
        results.extend(result_bmn)
...
def multi_gpu_test_bmn(model, data_loader, tmpdir=None, gpu_collect=True):
...
def single_gpu_test_cls(model, data_loader):
    """Test model with a single gpu.
...
def multi_gpu_test_cls(model, data_loader, tmpdir=None, gpu_collect=True):
    """Test model with multiple gpus.
