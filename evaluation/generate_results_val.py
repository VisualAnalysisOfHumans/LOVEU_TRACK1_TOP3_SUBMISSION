...
path_bmn_res1 = '../data/valres_csn_bmn_e12_0520/'
...
with open('eval_0507_1/k400_mr345_val_min_change_duration0.3.pkl', 'rb') as f:
    gt_dict = pickle.load(f, encoding='lartin1')

val_vids = []

# load output files
with open("val_predicts_4cls_ensemble_0.8gap.pkl",'rb') as f:
    pred_dict = pickle.load(f, encoding='lartin1')
    val_vids = list(pred_dict.keys())
print(len(val_vids))
...
def generate_part(val_vids_part, i):
    ...
    val_pkl_bmn1_1 = results_with_date + "val_predicts_bmn1_1_part" + str(i) + ".pkl"
       
    val_pkl_cls1 = results_with_date + "val_predicts_cls1_part" + str(i) + ".pkl"
    val_pkl_cls2 = results_with_date + "val_predicts_cls2_part" + str(i) + ".pkl"
    val_pkl_cls3 = results_with_date + "val_predicts_cls3_part" + str(i) + ".pkl"
    val_pkl_cls4 = results_with_date + "val_predicts_cls4_part" + str(i) + ".pkl"
    ...
    for vid_id in val_vids_part:
        ...
        if vid_id in vid2bias.keys() and vid2bias[vid_id] == 0:
            this_bias = 0
        else:
            this_bias = 1
...
for model in models:
    ...
    for part in range(25):
        model_path = results_with_date + "val_predicts_" + model + "_part" + str(part) + ".pkl"
    ...
    print(model, cn)
    output = open(results_with_date + "val_predicts_" + model + ".pkl", 'wb')
